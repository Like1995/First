...
public class BillDaoImpl implements BillDao {
	...
	public List<Bill> getBillList(Connection connection, Bill bill,int currentPageNo,int pageSize)
			throws Exception {
		// TODO Auto-generated method stub
		PreparedStatement pstm = null;
		ResultSet rs = null;
	...
	public int getBillCount(Connection connection, String productName,
			int providerId, int isPayment) throws Exception {
		...
		if(connection!=null){
			StringBuffer sql =new StringBuffer();
			sql.append("SELECT count(*) FROM smbms_bill AS b ,smbms_provider AS p WHERE b.providerId=p.id");
		...
		if(isPayment>0){
		 ...
		 rs=BaseDao.execute(connection, pstmt, rs, sql.toString(), params.toArray());
			while(rs.next()){
				count=rs.getInt(1);
			}
